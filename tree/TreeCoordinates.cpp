...
template <class T> struct Tmax {
  T value;
  Tmax(const T& value = -numeric_limits<T>::max()) : value(value) {}
  inline operator T() const { return value; }
  inline void add(const Tmax& other) { smax(value, other.value); }
  friend inline Tmax combine(const Tmax& lhs, const Tmax& rhs) {
    return max(lhs.value, rhs.value); }
};
namespace std {
  template <class T> struct numeric_limits<Tmax<T>> {
...
int main() {
  ...
  REP(i, m) {
    --p[i].X;
    --p[i].Y;
    sum[i] = lca.depth[p[i].X] + lca.depth[p[i].Y];
    if (sum[i] > sum[h]) h = i;
  }

  REP(i, m) {
    l[i].X = lca.depth[lca.lca(p[i].X, p[h].X)];
    l[i].Y = lca.depth[lca.lca(p[i].Y, p[h].Y)];
    o[i] = i;
  }
  sort(o, o + m, [](int i, int j) {
    return l[i].X > l[j].X || (l[i].X == l[j].X && l[i].Y < l[j].Y);
  });

  SegmentTree<Tmax<int>> tree(l[h].Y+1);
  int ans = 0;
  REP(i, m) {
    int j = o[i];
    int t = tree.query(0, l[j].Y+1);
    if (t >= -2*n) smax(ans, sum[j] - 2 * l[j].X + t);
    if (i > 0) smax(ans, sum[h] + sum[j] - 2 * (l[j].X + l[j].Y));
    tree.add(l[j].Y, sum[j] - 2 * l[j].Y);
  }
  writeln(ans);
